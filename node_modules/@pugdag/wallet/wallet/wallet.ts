...
import {PugdagAPI, ApiError} from './api';
...
const SompiPerPugdag = 100_000_000

// MaxSompi is the maximum transaction amount allowed in sompi.
const MaxSompi = 21_000_000 * SompiPerPugdag

export {pugdagcore, COMPOUND_UTXO_MAX_COUNT, CONFIRMATION_COUNT, COINBASE_CFM_COUNT};
...
class Wallet extends EventTargetImpl {
	...
	static networkTypes: Object = {
		...
		pugdagev: { port: 26889, network: 'pugdagev', name : 'devnet' }
	}
	...
	HDWallet: pugdagcore.HDPrivateKey;
	 ...
	 * Current network.
	 */
	network: Network = 'pugdag';

	api: PugdagAPI; //new PugdagAPI();
	...
	constructor(privKey: string, seedPhrase: string, networkOptions: NetworkOptions, options: WalletOptions = {}) {
		super();
		this.logger = CreateLogger('PugdagWallet');
		this.api = new PugdagAPI();
		//@ts-ignore
	...
	*/
	...
	composeTx({
		...
		try {
			const tx: pugdagcore.Transaction = new pugdagcore.Transaction()
				.from(utxos)
				.to(toAddr, amount)
				.setVersion(0)
				.fee(fee)
	...
	*/
	...
	async buildTransaction(txParamsArg: TxSend, debug = false): Promise < BuildTxResult > {
		...
		const ts_3 = Date.now();
		const inputs: RPC.TransactionInput[] = tx.inputs.map((input: pugdagcore.Transaction.Input) => {
			if (debug || this.loggerLevel > 0) {
		...
		const ts_4 = Date.now();
		const outputs: RPC.TransactionOutput[] = tx.outputs.map((output: pugdagcore.Transaction.Output) => {
			return {
